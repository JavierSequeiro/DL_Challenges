...
class ResBlock(nn.Module):
    def __init__(self, in_feat, out_feat, k_size=3, stride=1, padding=True, pool=False, batch_norm=True, act=nn.ReLU(inplace=True), pooling_layer=nn.MaxPool2d(2)):
        super(ResBlock, self).__init__()
        pad_cond = 1 if padding else 0
        self.conv = nn.Conv2d(in_feat, out_feat, kernel_size=k_size, stride=stride,padding=pad_cond)
        ...
        #                               self.bn)
        if in_feat != out_feat or stride != 1:
            skip_bn = nn.BatchNorm2d(out_feat) if batch_norm else nn.Identity()
            self.skip = nn.Sequential(
                nn.Conv2d(in_feat, out_feat, kernel_size=1, stride=stride),
                skip_bn
            )
        else:
            self.skip = nn.Identity()
...
class UpSampleBlock(nn.Module):
    def __init__(self, in_feat, out_feat, k_size, stride=1, res=False):
        super(UpSampleBlock, self).__init__()
        self.up=nn.ConvTranspose2d(in_channels=in_feat, out_channels=out_feat,kernel_size=k_size, stride=stride)

        if res:
            self.conv = nn.Sequential(ResBlock(in_feat, out_feat, k_size),
                                    ResBlock(out_feat, out_feat, k_size))
        else:
...
class DownSampleBlock(nn.Module):
    def __init__(self, in_feat, out_feat, res=False):
        super(DownSampleBlock, self).__init__()
        if res:
            self.conv = nn.Sequential(ResBlock(in_feat, out_feat, k_size=3, stride=1, padding=True, pool=False),
                                    ResBlock(out_feat, out_feat, k_size=3, padding=True, pool=True))
        else:
...
class UNet1(nn.Module):
    def __init__(self):
        ...
        self.d6 = DownSampleBlock(in_feat=1024, out_feat=2048)#16 
        self.up0 = UpSampleBlock(in_feat=2048, out_feat=1024)#32 
...
class ResUNet1(nn.Module):
    def __init__(self):
        ...
        # self.d1 = nn.Sequential(ConvBlock(in_feat=1, out_feat=64, k_size=3), 
        #                         ConvBlock(in_feat=64, out_feat=64, k_size=3)) #512
        self.d1 = nn.Sequential(ResBlock(in_feat=1, out_feat=64, k_size=3), 
                                ResBlock(in_feat=64, out_feat=64, k_size=3)) #512
        self.d2 = DownSampleBlock(in_feat=64, out_feat=128, res=True) #256
        self.d3 = DownSampleBlock(in_feat=128, out_feat=256, res=True) #128
        self.d4 = DownSampleBlock(in_feat=256, out_feat=512, res=True) #64
        self.d5 = DownSampleBlock(in_feat=512, out_feat=1024, res=True) #32
        self.d6 = DownSampleBlock(in_feat=1024, out_feat=2048, res=True)#16 
        self.up0 = UpSampleBlock(in_feat=2048, out_feat=1024, k_size=3, stride=1, res=True)#32 
        self.up1 = UpSampleBlock(in_feat=1024,out_feat=512, k_size=3, stride=1, res=True) #64
        self.up2 = UpSampleBlock(in_feat=512,out_feat=256, k_size=3, stride=1, res=True)# 128
        self.up3 = UpSampleBlock(in_feat=256,out_feat=128, k_size=3, stride=1, res=True) #256
        self.up4 = UpSampleBlock(in_feat=128,out_feat=64, k_size=3, stride=1, res=True) #512
        self.out = nn.Sequential(nn.Conv2d(64,1,kernel_size=1),
                                 nn.Sigmoid())
